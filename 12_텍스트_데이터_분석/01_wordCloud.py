...
많이 나올수록 크게, 작게 나올수록 작게 표시

쇼핑몰 리뷰 수천개 분석, 뉴스 키워드 파악, 유튭 댓글 패턴 분석, 설문 주관식 요약 등 사용
...
wc =WordCloud(
    ...
    prefer_horizontal=0.9,           #가로 단언 비율
    scale=1,                         #캔버스 비율
...
plt 내에 존재하는
interpolation = 보간법 = 알고있는 데이터 지점들을 기반으로 그 데이터 값을 추정하여
                            데이터의 공백 채우거나 곡선을 생성하는 데이터 처리 기법
...
def 기본문법():
    ...
    wc = WordCloud() # 커스텀을 안할경우, 기본값으로 세팅된다

    # 2. .generate("문자데이터")
    # 문자열을 받아 단어 빈도를 자동으로 계산하고 WordCloud를 만들어주는 기능
    # 문자열을 공백 기준으로 단어 분리
    # 각 단어 빈도 자동 계산
