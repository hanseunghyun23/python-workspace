...
def url목록가져오기():
    랭킹주소 = "https://news.daum.net/ranking/popular"
    ...
    for 태그 in 태그들[:20]:  # 최대 20개
        ...
        링크 = requests.get(랭킹주소,headers=headers)
...
def 기사수집(url):
    ...
    # TODO: 기자 태그 찾기
    기자태그 = soup.find("h3", class_="txt_info")

    # TODO: 날짜 태그 찾기
    날짜태그 = soup.find("span", class_ ="info_view" )

    # TODO: 각 변수에 텍스트 값 저장하기 (없으면 "못 찾음" 출력)
    제목 = 아티클.title
    내용 = 아티클.text[100] + "..."   # 내용은 100자만
    기자 = 기자태그.text.strip() if 기자태그 else "못찾음"
    날짜 = 날짜태그.text.strip() if 기자태그 else "못찾음"
