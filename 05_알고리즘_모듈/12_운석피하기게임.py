...
# 이미지 불러오기
배경이미지 = pygame.transform.scale(pygame.image.load("images/running/background.png").convert(), (800, 600))
플레이어이미지 = pygame.transform.scale(pygame.image.load("images/running/player.png").convert_alpha(), (60, 70))
금화이미지 = pygame.transform.scale(pygame.image.load("images/running/hp.png").convert_alpha(), (40, 40))

# 플레이어 설정
플레이어_x = 400
플레이어_y = 500
플레이어_속도 = 6   # TODO 1: 플레이어 이동 속도 (힌트: 1~10 사이 숫자)

# 운석 설정
운석목록 = []
운석속도 = 4       # TODO 2: 운석 떨어지는 속도 (힌트: 1~6 사이 숫자)

# 금화 설정
금화목록 = []
금화속도 = 3        # TODO 3: 금화 속도 (힌트: 운석보다 느리게)
...
while 실행중:
    ...
    if not 게임오버:
        ...
        if 금화타이머 >= random.randint(60, 120):
            ...
            # 운석 이동
        for 운석 in 운석목록:
            운석[1] += 5                                 # TODO 10: 운석속도 변수
            운석목록[:] = [o for o in 운석목록 if o[1] < 650]

                # 금화 이동
        for 금화 in 금화목록:
            금화[1] += 4                                 # TODO 11: 금화속도 변수
