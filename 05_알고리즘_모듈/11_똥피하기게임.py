...
# 이미지 불러오기
플레이어이미지 = pygame.transform.scale(pygame.image.load("player.").convert_alpha(), (70, 70))
똥이미지 = pygame.transform.scale(pygame.image.load("poop.png").convert_alpha(), (40, 40))
...
while 실행중:
    ...
    if not 게임오버:
        ...
        for 똥 in 똥목록:
            똥[1] += 10                                  # TODO 8: 똥 아래로 이동 (똥속도 변수)

            똥목록[:] = [똥 for 똥 in 똥목록 if 똥[1] < 630] # TODO 9: 화면 밖 기준값
            ...
            for 똥 in 똥목록:
                ...
                점수 += 1
                if 점수 % 200 == 0: 똥속도 += 15               # TODO 12: 난이도 증가

                # 화면 그리기
                화면.fill(검정)                                        # TODO 13: 배경색
