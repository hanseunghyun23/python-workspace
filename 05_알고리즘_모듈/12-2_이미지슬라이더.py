...
class 이미지회전앱:
    ...
    def UI만들기(self):
        ...
        tk.Button(프레임, text="저장", command="이미지저장", **btn).grid(row=0, column=3, padx=8)

        # TODO 2: 아래 주석을 해제하고 command=??? 를 채우세요
         tk.Button(프레임, text="초기화", command="초기화", **btn).grid(row=1, column=0, padx=8, pady=8)

        # TODO 3: 아래 주석을 해제하고 command=??? 를 채우세요
         tk.Button(프레임, text="↔ 좌우반전", command="좌우반전", **btn).grid(row=1, column=1, padx=8, pady=8)
