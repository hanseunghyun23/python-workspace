...
MarkerCluster()
 #folium_excel_클러스터링 위해 추가한 라이브러리 모듈
# folium 큰 상자 안에
##      plugins에서
...
#하지만 from folium.plugins import MarkerCluster 를 작성하지 않으면 너무 길게 작성하게 되므로
#from folium.plugins import MarkerCluster 를 작성해서 MarkerCluster() 으로 줄여서 사용하겠다
### 만약 as 를 붙이고 싶다면
...
def folium_excel_위도경도():
    ...
    df = pd.read_excel('공공자전거_대여소_정보(25.12월_기준).xlsx', header=None, skiprows=4)
...
def folium_excel_클러스터링():
    ...
    m.save("따릉이지도.html")
#   위도 경도를 마커가 너무 많으면 동그라미 표기해주는 곳에 서팅할 거고
#   동그러미 표기법은 folium에서 제공하는 지도 형태를 따를 것이ㅣ다
    # ValueError: Location values cannot contain NaNs.
    # NaN 때문에 발생한 상황 위도 경도가 비어있는 행이 있다.
    # 마커 찍기를 하기 전에 위도 경도가 비어있는 행을 버리거나 0 으로 만들거나
    # 기획자와 회사가 결정한 사항에 따라서 처리
