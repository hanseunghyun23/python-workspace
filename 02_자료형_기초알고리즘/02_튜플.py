...
#서버설정[0] = "localhost"
#print(서버설정[0]) 튜플은 상수처럼 변경할 수 없다. 하위에서 절대 변경하면 안될경우 사용
# java = final, record=여러 값을 묶ㅇ서 상수 처리  javaScript = const 처럼 python()로 표기
...
# List 장바구니 플레이리스트 Tuple  서버설정 GPS 좌표 색상표와 같이 고정적인 값에서 주로 사용