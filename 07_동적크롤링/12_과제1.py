...
def 크롤링실행(검색어, 상태라벨):
    ...
    상태라벨.config(text="검색중...")
    상태라벨.update()

    # TODO 1-2: 이미지 저장 폴더를 만드세요
    os.makedirs("나무위키이미지저장", exist_ok=True)
    ...
    df.to_csv(검색어, index=False, encoding="csv")
...
tk.Button(창, text="검색시작", command=버튼클릭, font=("맑은 고딕", 11)).pack(pady=10)
